...
void TestSmallGraph() {
  ...
  vector<MinCutEdge<int>> min_cut_edges = compute_min_cut<int>(&my_g);
  EXPECT_EQ(1, min_cut_edges.size(),
      "Small min cut should contain single edge.");
  EXPECT_EQ(1, min_cut_edges[0].edge.capacity,
...
void TestMediumGraph() {
  ...
  Graph<int,EdgeCapacity,false> my_g = Graph<int,EdgeCapacity,false>(
      ...
      {1.0,3.0,2.0,3.0,2.0}});
  vector<MinCutEdge<int>> min_cut_edges = compute_min_cut<int>(&my_g);
  EXPECT_EQ(2, min_cut_edges.size(),
      "Medium min cut should contain two edges.");
  double min_cut_cost = 0.0;
  for (size_t i = 0; i < min_cut_edges.size(); i++) {
    min_cut_cost += min_cut_edges[i].edge.capacity; 
  }
  EXPECT_EQ(4.0, min_cut_cost,
...
void TestLargeGraph() {
  ...
  Graph<int,EdgeCapacity,false> my_g = Graph<int,EdgeCapacity,false>(
      ...
      {{0,4},{0,3},{0,2},{4,3},{3,2},{2,5},{3,6},{4,6},{5,6},{4,8},{5,7},{6,7},{6,8},{7,8},{7,9},{7,10},{8,10},{8,11},{9,1},{10,1},{11,1}},
      {9.0,6.0,3.0,4.0,3.0,7.0,6.0,2.0,5.0,8.0,1.0,1.0,7.0,1.0,3.0,5.0,2.0,1.0,5.0,4.0,3}});
  vector<MinCutEdge<int>> min_cut_edges = compute_min_cut<int>(&my_g);
  EXPECT_EQ(5, min_cut_edges.size(),
      "Large min cut should contain five edges.");
  double min_cut_cost = 0.0;
  for (size_t i = 0; i < min_cut_edges.size(); i++) {
    min_cut_cost += min_cut_edges[i].edge.capacity; 
  }
  EXPECT_EQ(6.0, min_cut_cost,
